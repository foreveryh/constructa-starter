...
async function testMultiTurn() {
  ...
  // 第一轮对话
  console.log("\n📝 第一轮: 'My favorite color is blue. Remember it.'");
  console.log("-".repeat(60));

  const stream1 = query({
    prompt: "My favorite color is blue. Remember it.",
    options: {
      cwd: process.cwd(),
      model,
  ...
  // 第二轮对话 - 使用 resume
  console.log("\n" + "=".repeat(80));
  console.log("\n📝 第二轮 (resume): 'What is my favorite color?'");
  console.log("-".repeat(60));
  ...
  for await (const event of stream2) {
    ...
    if (type === "assistant") {
      ...
      if (text) {
        response2 = text;
        mentionsBlue = text.toLowerCase().includes("blue");
    ...
    if (type === "result") {
      ...
      console.log(`✅ 第二轮完成`);
      if (res.result) {
        mentionsBlue = mentionsBlue || res.result.toLowerCase().includes("blue");
      }
    }
  }
  ...
  console.log(`  记住了颜色: ${mentionsBlue ? "✅ 是 (提到了 blue)" : "❌ 否"}`);

  const success = sessionId === newSessionId && mentionsBlue;
  console.log(`\n  总体结果: ${success ? "✅ 多轮对话测试通过" : "❌ 多轮对话测试失败"}`);
}
